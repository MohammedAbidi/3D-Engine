...
public:
	...
	virtual int rd_option_array(const string & name, int n, const vector<float> &values);
...
class REDirect: public RenderEngine
{
 public:
	...
	int rd_bezier_patch(const string & vertex_type,
					int u_degree, int v_degree, const vector<float> & vertex);
...
// Returns the number of components in an attributed vertex type
	...
	{
		bool geom_flag;  // Technically always set
		bool color_flag;
		bool normal_flag;
		bool texture_flag;
		bool weight_flag;
...
#define DRAW        1
	...
	int line_pipeline(attr_point& p, int draw_flag);
	// Takes a point in object coordinates and a MOVE/DRAW flag and
	// treats it as the endpoint of a line to be drawn.
